...
- (void)reloadData{
    ...
    UITapGestureRecognizer *cellTapRec = nil;
    for (NSInteger i = 0; i < count; i++) {
        ...
        cellContainer.tapDeleteHandler = [self buildTapDeleteHandlerForIndex:i];
        ...
        cell.frame = CGRectMake((_cellContainerSize.width - _cellSize.width) / 2,
                                ...
                                _cellSize.width,
                                _cellSize.height);
        cellTapRec = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(onTapCell:)];
        [cell addGestureRecognizer:cellTapRec];
        ...
        cellContainer.deleteButton.center = CGPointMake(cell.frame.origin.x + deleteButtonSize.width / 6.0, cell.frame.origin.y + deleteButtonSize.height / 6.0);
...
#pragma mark - 删除元素
- (M2SGVIC_TapDeleteHandler)buildTapDeleteHandlerForIndex:(NSInteger)index{
    if (!_delegate
        || ![_delegate respondsToSelector:@selector(gridView:shouldDeleteCellAtIndex:)]
        || ![_delegate respondsToSelector:@selector(gridView:wantsDeleteCellAtIndex:)]) {
        return nil;
    }
    
    __weak typeof(self) weakSelf = self;
    M2SGVIC_TapDeleteHandler tapDeleteHandler = ^(M2SimpleGridViewCellContainer * deleteCell){
        if (![weakSelf.delegate gridView:self shouldDeleteCellAtIndex:index]) {
            return;
        }
...
- (void)onTapCell:(UITapGestureRecognizer *)tapRec{
    if (self.isEditing) {
        return;
    }
    UIView *cell = tapRec.view;
    M2SimpleGridViewCellContainer *container = (M2SimpleGridViewCellContainer *)[cell superview];
    NSInteger index = [_cells indexOfObject:container];
    if (_delegate && [_delegate respondsToSelector:@selector(gridView:didSelectedCellAtIndex:)]) {
        [_delegate gridView:self didSelectedCellAtIndex:index];
    }
}
